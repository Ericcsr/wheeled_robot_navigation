...
def rapidlyExploringRandomTree(ax, img, start, goal, seed=None):
    ...
    if goal in points:
        ...
        print
        'Showing resulting map'
        print
        'Final path:', path
        print
        'The final path is made from:', len(path), 'connected points'
    else:
        path = None
        print
        'Reached maximum number of vertex and goal was not found'
        print
        'Total vertex in graph:', len(points), 'total random points generated:', i
        print
        'Showing resulting map'
    ppl.title("Taken:{0} second to converge".format(time.time() - ls_time))
    ppl.show()
    return path
