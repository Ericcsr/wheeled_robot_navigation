...
STEP_DISTANCE = 5+3*(rank%5)            # Different sample size assumptions
SEED = rank                             # For random numbers

# Package send from server to worker
class Workpack:
    def __init__(self,map,start,goal):
        self.image = map
        self.points = (start,goal)

# Package send from worker to server
class Resultpack:
...
def rapidlyExploringRandomTree(img, start, goal, seed=None):
    ...
    i = 0
    while (goal not in points) and (len(points) < conf.MAX_NUM_VERT) and i <= 100000:
        if (i % 100) == 0:
            print
...
# Select start point and goal from GUI
...
# Parallel worker task to plan the path
def worker_task():
    while True:
...
# The task of running the main logic flow
def server_task():
    # === Prepare the local necessities ===
    ...
    # === Get the goal from user interface ===
        ...
        while t*step < desired_time :
            ...
            for i in range(len(X)):
                ...
                path_image = cv.resize(path_image,(2*path_image.shape[1],2*path_image.shape[0]),
                             cv.INTER_LINEAR)
            # ============================
            cum_v[t%10] = math.sqrt(V[0][0]**2+V[0][1]**2)
            if sum(cum_v)/10 <= REPLAN_V_THRESH:
                break
