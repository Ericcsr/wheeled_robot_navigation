...
def manh_dist(pose1,pose2):
    """Compute the manhattan distance, faster"""
    return abs(pose1[0]-pose2[0])+abs(pose1[1]-pose2[1])

def RVO_update(X, V_des, V_current, ws_model,local = 1.5):
...
class Tools:
    def __init__(self,n,thresh):
        self.visited = [0]*n
        self.thresh = thresh
    def get_goal(self, X, goals):
        goal = [None,None,None,None]
        for i,x in enumerate(X):
            l = goals[i][self.visited[i]:]
            for j,iter in enumerate(l):
                if distance(x,iter) > self.thresh and j==0:
                    goal[i] = iter
                    break
                else :
                    goal[i] = l[j]
                    if self.visited[i] < len(goals[i])-1:
                        self.visited[i] += 1
                    break
        return goal

    def reset_goal(self,agent):
        self.visited[agent] = 0
